...
class GetFullNeighbor: public AsyncOpKernel {
 public:
  explicit GetFullNeighbor(OpKernelConstruction* ctx): AsyncOpKernel(ctx) {
    ...
    ss.str("");
    if (!condition_.empty()) {
      ss << "v(nodes).outV(edge_types).has(" << condition_ << ").as(nb)";
      query_str_ = ss.str();
    } else {
      query_str_ = "v(nodes).outV(edge_types).as(nb)";
    }
  }
 private:
...
void GetFullNeighbor::ComputeAsync(OpKernelContext* ctx, DoneCallback done) {
...
REGISTER_KERNEL_BUILDER(
    Name("GetFullNeighbor").Device(DEVICE_CPU), GetFullNeighbor);
